 ...
 * notifications are optional infra, never a hard dependency for the
 * calling kit's real event, e.g. `advanceOrder`/`redeemAction`) and a
 * fetch failure is caught and logged rather than propagated. Callers that
 * need the send outcome should not rely on this; it deliberately has no
