...
const generateLinkToken = vi.fn().mockReturnValue("generated-token-xyz");
...
function makeRequest(body: unknown): Request {
  return new Request(
    "https://merqo.example.com/api/merqo/vendor-connect-token",
    {
      method: "POST",
...
beforeEach(() => {
  ...
  generateLinkToken.mockReturnValue("generated-token-xyz");
  insert.mockClear();
...
describe("POST /api/merqo/vendor-connect-token", () => {
  ...
  it("mints a kind='vendor' token, inserts telegram_link_tokens with a 30-minute expiry, and returns the deep link", async () => {
    ...
    expect(json).toEqual({
      ...
      deep_link: "https://t.me/MerqoNotifyBot?start=generated-token-xyz",
    });
  ...
  it("400s on a malformed body (wrong type)", async () => {
    const res = await POST(makeRequest({ vendor_id: 12345, kit_slug: "qkit" }));
    expect(res.status).toBe(400);
    expect(from).not.toHaveBeenCalled();
  });
