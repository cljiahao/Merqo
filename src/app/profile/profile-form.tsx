...
export function ProfileForm({
  ...
  // Stall name (shared merqo.vendor_profile) — persisted via a server action.
  ...
  // the storage upload; we persist the returned URL on the auth user, same
  // channel as the display name. null clears it and the initials badge
  // takes over.
  ...
  // Social/website links (shared merqo.vendor_profile).
  ...
  return (
    // Two independent flex-column stacks, not a CSS grid — see the cross-kit
    // profile-page standard §2.3: a grid's row height tracks the tallest
    // cell in that row, so once a multi-field section (Social & website)
    // outgrows a single-input one (Stall name), every row after it starts
    // late in both columns. Column order (§2.4): stall name, profile
    // picture, change password | display name, social links.
