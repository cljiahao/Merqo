...
- `legal/` — public legal document pages (`terms/`, `privacy/`, `pilot-agreement/`, `end-customer-notice/`), each a one-line Server Component page rendering `@merqo/ui`'s `LegalDocument` with the matching `doc` prop (`"terms"`, `"privacy"`, `"pilot"`, `"end-customer-notice"`). Linked from the landing footer (`LegalFooterLinks`) and reachable directly; `end-customer-notice/` is linked instead from merqo's own Telegram bot. `accept/` is the interstitial `requireVendorSession` (`src/lib/vendor.ts`) redirects a signed-in vendor to when their terms/privacy acceptance is stale — `page.tsx` reads the `next` search param and renders the client `accept-form.tsx` (`@merqo/ui`'s `TermsAcceptanceCheckbox`, just the agree checkbox, submit disabled until it's checked); `actions.ts`'s `acceptLegalTerms` server action reads the vendor's real `ip`/`user_agent`, inserts both `legal_acceptances` rows (service-role client — `authenticated` only has select on that table) and redirects to `next`, tolerating a `23505` duplicate insert on either row as success independently. Deliberately does not call `requireVendorSession` itself or any other legal-gate check — that would be the gate redirecting to the page it exists to satisfy, an infinite loop. See `legal/README.md` for the full breakdown.
- `login/` — email/password sign-in.
- `no-access/` — shown when a signed-in user has no kit access and no admin role.
- `not-found.tsx` — branded 404.
- `page.tsx` — `Home` async server component, the public brand landing page. Composes `Nav`, `Hero`, `Benefits`, `KitStacker`, `HowItWorks`, `Faq`, `Footer`, and `BackToTop` from `@/components/landing/`. No CTA band above the footer, matching qkit. Its own sticky mobile CTA, like every landing CTA, links a signed-in vendor to `/post-login` (never straight to `/admin`) so a non-team vendor doesn't get bounced to `/no-access`.
