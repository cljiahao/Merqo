...
- `legal/` — public legal document pages (`terms/`, `privacy/`, `pilot-agreement/`), each a one-line Server Component page rendering `@merqo/ui`'s `LegalDocument` with the matching `doc` prop (`"terms"`, `"privacy"`, `"pilot"`). Linked from the landing footer (`LegalFooterLinks`) and reachable directly. `accept/` is the interstitial `requireVendorSession` (`src/lib/vendor.ts`) redirects a signed-in vendor to when their terms/privacy acceptance is stale — `page.tsx` reads the `next` search param and renders the client `accept-form.tsx` (`@merqo/ui`'s `TermsAcceptanceCheckbox`, submit disabled until both fields are filled); `actions.ts`'s `acceptLegalTerms` server action inserts both `legal_acceptances` rows (service-role client — `authenticated` only has select on that table) and redirects to `next`, tolerating a `23505` duplicate insert as success. Deliberately does not call `requireVendorSession` itself or any other legal-gate check — that would be the gate redirecting to the page it exists to satisfy, an infinite loop.
