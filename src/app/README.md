...
- `legal/` — public legal document pages (`terms/`, `privacy/`, `pilot-agreement/`), each a one-line Server Component page rendering `@merqo/ui`'s `LegalDocument` with the matching `doc` prop (`"terms"`, `"privacy"`, `"pilot"`). Linked from the landing footer (`LegalFooterLinks`) and reachable directly.
