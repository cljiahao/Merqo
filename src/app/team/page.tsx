...
export default async function TeamPage() {
  ...
  const results = await Promise.all(products.map((p) => fetchProductMetrics(p)));
  ...
  return (
    <main className="mx-auto max-w-5xl p-6">
      ...
      <p className="mt-1 text-sm text-gray-500">
        ...
        {totals.products_down > 0 ? `, ${totals.products_down} unavailable` : ""}
      </p>
      ...
      <section className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <Stat label="Upgrade requests" value={String(totals.pending_upgrade_requests)} />
      </section>
