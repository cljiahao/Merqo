...
// auto-runs at most once per user, so statically importing it would ship the
// library in every dashboard page's bundle for nothing. The type import above
// is erased at compile time (no runtime cost).
 ...
 * driver.js overlay. Auto-runs once on the dashboard welcome landmark for
 * users who haven't seen it (server-tracked via `seen`); the button replays
 * it anytime.
 */
export function DashboardTour({ seen }: { seen: boolean }) {
  ...
  // a single time, and a replay by an already-seen user never re-stamps.
  const seenRef = useRef(seen);
  // Set when replay is tapped from another page; the tour then runs once we
  // land back on the dashboard overview (step 1's anchor).
  ...
  function onReplay() {
    if (pathname !== "/dashboard") {
      // Step 1 spotlights the dashboard welcome landmark, so get there first.
