...
if echo "$scan" | grep -qE '\bgit\b' && echo "$scan" | grep -qE '\b(commit|push|config)\b' && echo "$scan" | grep -qE '(^|[[:space:]])HUSKY=0|(^|[[:space:]])HUSKY_SKIP_HOOKS=|core\.hooksPath'; then
  echo "BLOCKED: HUSKY=0 / HUSKY_SKIP_HOOKS / a core.hooksPath override ('git -c core.hooksPath=...', 'git config core.hooksPath=...', 'git config --unset core.hooksPath') disables the pre-commit/pre-push hook layer — the same bypass as --no-verify. Fix the failure instead." >&2
  exit 2
fi
