...
test.describe("authed areas", () => {
  test.skip(process.env.MERQO_E2E_AUTH !== "1", "needs seeded auth (set MERQO_E2E_AUTH=1)");
  ...
  test("vendor catalog renders with a join-waitlist control", async ({ page }) => {
    await page.goto("/products");
    await expect(page.getByRole("heading", { name: "Your Merqo products" })).toBeVisible();
