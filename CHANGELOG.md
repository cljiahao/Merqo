...
- Bumped `vitest` and `@vitest/coverage-v8` to `4.1.11` (from `3.2.6`).
  ...
  `fast-uri` / `qs` advisories other kits also cleared do not reach this
  tree. Full suite green on vitest 4 with no test or config changes.
...
- Bumped `next` to `16.3.4` (`eslint-config-next` to match) and refreshed
  ...
  (GHSA-p293-qw3h-jr36, GHSA-2xp9-vwfh-vxw4) and two high `browserslist`
  advisories (GHSA-73wf-gq98-2v4g and its sibling). merqo already builds
  without `output: "standalone"`, so no config change was needed.
...
- Cards were visually indistinguishable from the page background in both
  ...
  distinct card treatment in both modes (`src/app/globals.css`); both
  deltas were widened further in follow-up passes after reading as too
  subtle at a glance.
