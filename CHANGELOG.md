...
- **templateCentral 5.12 harness migration.** `prepare` script now tolerates
  a missing `.git` (`lefthook install || true`) so Docker builds that exclude
  `.git` via `.dockerignore` don't hard-fail on `pnpm install`. `next` pin
